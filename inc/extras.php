...
function _categorized_blog() {
	...
	// Get the categories.
	$all_the_cool_cats = get_transient( '_categories' );
 ...
 * @return string
 */
function _get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( __( 'Width must be an integer', 'xyz' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( __( 'Height must be an integer', 'xyz' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for background_color', 'xyz' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for text_color', 'xyz' ) );
	}
	...
	$alt = apply_filters( '_placeholder_image_alt', __( 'WebDevStudios Placeholder Image', 'xyz' ) );
 ...
 * @return string
 */
function _get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
		'category'			=> '',
